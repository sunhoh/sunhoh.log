...
const PostNavigation = ({ prevPost, nextPost }:PostNavigationProps) => {
    ...
    return (
        <div className='flex items-center justify-between text-sm'>
          ...
          {prevPost ? (
            <Link href={`/posts/${prevPost.postId}`} className='flex flex-col max-w-[60%] tracking-tight hover:brightness-125 '>
                <div className='mb-2 font-light text-gray-400'>Prev</div>
                <span>{prevPost.title}</span>
            </Link>
          ): <div />}
          {nextPost ? (
            <Link href={`/posts/${nextPost.postId}`} className='flex flex-col ml-auto max-w-[60%] tracking-tight text-right hover:brightness-125'>
